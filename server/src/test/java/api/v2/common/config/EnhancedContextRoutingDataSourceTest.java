...
import api.v2.common.config.EnhancedContextRoutingDataSource;
...
class EnhancedContextRoutingDataSourceTest {
        ...
        private TestableEnhancedContextRoutingDataSource routingDataSource;

        // 테스트를 위한 확장 클래스
        private static class TestableEnhancedContextRoutingDataSource extends EnhancedContextRoutingDataSource {
                @Override
        ...
        void setUp() {
                // 각 테스트 전 초기화
                routingDataSource = new TestableEnhancedContextRoutingDataSource();
        ...
        void 멀티_스레드_환경에서_서비스별_라우팅이_독립적으로_동작한다() throws InterruptedException {
                ...
                for (int i = 0; i < 3; i++) {
                        ...
                        threads[i] = new Thread(() -> {
                                ...
                                System.out.println("           │ \033[34m🧵 Thread-" + (index + 1)
                                                + "\033[0m: serviceId=\033[36m"
                                                + serviceIds[index] + "\033[0m → lookupKey=\033[32m" + results[index]
                                                + "\033[0m");
                        });
                }
