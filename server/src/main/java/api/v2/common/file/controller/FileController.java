package api.v2.common.file.controller;

import io.swagger.v3.oas.annotations.tags.Tag;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import api.v2.common.dto.ApiResponseSchema;
import api.v2.common.file.dto.FileDto;
import api.v2.common.file.entity.CmsFile;
import api.v2.common.file.service.FileService;
...
public class FileController {
    ...
    public ResponseEntity<ApiResponseSchema<?>> uploadFiles(
        ...
        log.debug("Received 'file' part: {}",
                (singleFile != null && !singleFile.isEmpty() ? singleFile.getOriginalFilename() : "empty or null"));
        ...
        try {
            ...
            if (singleFile != null && !singleFile.isEmpty()) {
                boolean alreadyAdded = fileList.stream()
                        .anyMatch(existingFile -> existingFile.getOriginalFilename()
                                .equals(singleFile.getOriginalFilename()) &&
                ...
                if (!alreadyAdded) {
                    ...
                    log.debug("Single file in 'file' part already processed via 'files' part: {}",
    ...
    public ResponseEntity<ApiResponseSchema<?>> getAllFiles(
        ...
        try {
            ...
            log.error("Failed to get all files for menu: {}, publicYn: {}, page: {}, size: {}", menu, publicYn, page,
                    size, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponseSchema.error(
    ...
    private ResponseEntity<?> buildFileResponse(Long fileId, boolean inlineDisposition) {
        try {
            ...
            } catch (IOException | UnsupportedOperationException ex) { // Added UnsupportedOperationException for
                                                                       // certain Resource types
                log.info(
                        "Could not determine file type for {} via probeContentType. Relying on stored MIME type or defaulting. Error: {}",
                        fileInfo.getSavedName(), ex.getMessage());
            ...
            String encodedFileName = URLEncoder.encode(fileInfo.getOriginName(), StandardCharsets.UTF_8.toString())
                    .replaceAll("\\+", "%20");
            ...
            // Add Cache-Control header to prevent caching of sensitive files if needed, or
            // allow caching for public images
            // For example, for public images that change infrequently:
            // headers.setCacheControl(CacheControl.maxAge(3600,
            // TimeUnit.SECONDS).mustRevalidate().cachePublic());
