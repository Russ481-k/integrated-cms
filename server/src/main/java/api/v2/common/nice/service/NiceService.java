package api.v2.common.nice.service;
...
import api.v2.common.nice.dto.NiceCallbackResultDto;
import api.v2.common.nice.dto.NiceErrorDataDto;
import api.v2.common.nice.dto.NiceReqSeqDataDto;
import api.v2.common.nice.dto.NiceUserDataDto;
...
public class NiceService {
    ...
    public String storeSuccessData(String encodeData) {
        ...
        log.info("[NICE] storeSuccessData - Successfully validated reqSeq: {} for serviceType: {}", reqSeq,
                serviceType);
        ...
        switch (serviceType) {
            ...
            case "FIND_ID":
                ...
                if (userOptional.isPresent()) {
                    ...
                    if (foundUser.getEmail() != null && !foundUser.getEmail().isEmpty()) {
                        try {
                            emailService.sendUserIdEmail(foundUser.getEmail(), foundUser.getUsername(),
                                    foundUser.getName());
                            log.info("[NICE] FIND_ID - User ID email sent to: {} for user: {}", foundUser.getEmail(),
                                    foundUser.getUsername());
                            callbackResult = NiceCallbackResultDto.idFound(serviceType, foundUser.getEmail(),
                                    nameFromNice);
                        } catch (Exception e) {
                            log.error("[NICE] FIND_ID - Failed to send User ID email to: {}", foundUser.getEmail(), e);
                            callbackResult = NiceCallbackResultDto.error(serviceType, "EMAIL_SEND_FAILED",
                                    "아이디 안내 메일 발송에 실패했습니다. 관리자에게 문의해주세요.", nameFromNice);
                        }
                    } else {
                        log.warn("[NICE] FIND_ID - User {} has no email address. Cannot send ID.",
                                foundUser.getUsername());
                        callbackResult = NiceCallbackResultDto.error(serviceType, "NO_EMAIL",
                                "사용자에게 이메일 주소가 등록되어 있지 않아 아이디를 발송할 수 없습니다.", nameFromNice);
                    }
                } else {
                ...
                if (userOptional.isPresent()) {
                    ...
                    if (userToReset.getEmail() != null && !userToReset.getEmail().isEmpty()) {
                        ...
                        try {
                            ...
                            emailService.sendTemporaryPasswordEmail(userToReset.getEmail(), tempPassword,
                                    userToReset.getName());
                            log.info("[NICE] RESET_PASSWORD - Temporary password email sent to: {} for user: {}",
                                    userToReset.getEmail(), userToReset.getUsername());
                            callbackResult = NiceCallbackResultDto.passwordResetSent(serviceType,
                                    userToReset.getEmail(), nameFromNice);
                        } catch (EmailSendingException e) {
                            log.error(
                                    "[NICE] RESET_PASSWORD - Failed to send temporary password for user: {}. ErrorCode: {}, Message: {}",
                                    userToReset.getUsername(), e.getErrorCode(), e.getMessage(), e);
                            callbackResult = NiceCallbackResultDto.error(serviceType, e.getErrorCode(), e.getMessage(),
                                    nameFromNice);
                        } catch (Exception e) {
                            log.error("[NICE] RESET_PASSWORD - Failed to process password reset for user: {}",
                                    userToReset.getUsername(), e);
                            callbackResult = NiceCallbackResultDto.error(serviceType, "PASSWORD_RESET_PROCESS_FAILED",
                                    "비밀번호 재설정 처리 중 오류가 발생했습니다. 관리자에게 문의해주세요.", nameFromNice);
                        }
                    } else {
                        log.warn(
                                "[NICE] RESET_PASSWORD - User {} has no email address. Cannot send temporary password.",
                                userToReset.getUsername());
                        callbackResult = NiceCallbackResultDto.error(serviceType, "NO_EMAIL",
                                "사용자에게 이메일 주소가 등록되어 있지 않아 임시 비밀번호를 발송할 수 없습니다.", nameFromNice);
                    }
                } else {
            ...
            default:
                ...
                callbackResult = NiceCallbackResultDto.error(serviceType, "INVALID_SERVICE_TYPE",
                        "알 수 없는 서비스 요청 유형입니다.", nameFromNice);
                break;
        }
        ...
        log.info("[NICE] Stored callback result for serviceType: {} with resultKey: {}, expiry: {} ({} mins)",
                serviceType, resultKey, expiryTime, RESULT_EXPIRY_MINUTES);
        return resultKey;
    }
    ...
    public String storeErrorData(String encodeData) {
        ...
        if (reqSeqData != null) {
            ...
            log.info("[NICE] storeErrorData - reqSeq: {} has serviceType: {}. Storing error data.", reqSeq,
                    serviceType);
        } else {
            log.warn(
                    "[NICE] storeErrorData - reqSeq not found or expired: {}. Error data will be stored without serviceType context.",
                    reqSeq);
        }
        ...
        log.info(
                "[NICE] Stored ERROR data with resultKey: {}, reqSeq: {}, errorCode: {}, serviceType: {}, expiry: {} ({} mins)",
    ...
    public NiceUserDataDto getVerifiedNiceUserDataForRegister(String resultKey) {
        ...
        if (data instanceof NiceCallbackResultDto) {
            ...
            if ("REGISTER".equals(callbackResult.getServiceType()) && callbackResult.getUserData() != null) {
                ...
                log.error(
                        "[NICE] Cached data for resultKey: {} is NiceCallbackResultDto but not for REGISTER or userData is null. ServiceType: {}",
                        resultKey, callbackResult.getServiceType());
                throw new RuntimeException("NICE 인증 결과가 회원가입 성공 데이터가 아닙니다.");
            }
        } else if (data instanceof NiceUserDataDto) {
            tempResultStore.remove(resultKey);
            log.warn(
                    "[NICE] Directly consumed NiceUserDataDto for resultKey: {} (should be wrapped in NiceCallbackResultDto for REGISTER)",
                    resultKey);
            return (NiceUserDataDto) data;
        }
        log.error("[NICE] Cached data for resultKey: {} is not expected type for REGISTER. Actual type: {}", resultKey,
    ...
    public Object peekRawNiceData(String resultKey) {
        ...
        log.info("[NICE] peekRawNiceData - Successfully retrieved data for resultKey: {} (without consuming)",
                resultKey);
    ...
    private void storeReqSeqWithServiceType(String reqSeq, String serviceType) {
        ...
        log.info("[NICE] Stored reqSeq: {} with serviceType: {}, expiry: {} ({} mins)", reqSeq, serviceType, expiryTime,
                REQ_SEQ_EXPIRY_MINUTES);
    }
    ...
    public NiceReqSeqDataDto consumeAndValidateReqSeq(String reqSeq) {
        ...
        tempReqSeqStore.remove(reqSeq);
        log.info(
                "[NICE] consumeAndValidateReqSeq - Successfully validated and consumed reqSeq: {}. tempReqSeqStore size after removal: {}",
                reqSeq, tempReqSeqStore.size());
        if (entry.data instanceof NiceReqSeqDataDto) {
            return (NiceReqSeqDataDto) entry.data;
        } else {
            log.warn(
                    "[NICE] consumeAndValidateReqSeq - Data in tempReqSeqStore for reqSeq: {} is not of type NiceReqSeqDataDto. Actual type: {}. Returning null.",
    ...
    private NiceReqSeqDataDto getReqSeqData(String reqSeq) {
        ...
        if (entry.data instanceof NiceReqSeqDataDto) {
            ...
            log.warn(
                    "[NICE] getReqSeqData - Data in tempReqSeqStore for reqSeq: {} is not of type NiceReqSeqDataDto. Actual type: {}. Returning null.",
                    reqSeq, entry.data != null ? entry.data.getClass().getName() : "null");
            return null;
        }
    }
