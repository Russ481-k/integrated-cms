...
public class CustomUserDetailsServiceImpl implements CustomUserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        if (customUserDetails != null && customUserDetails.getUser() != null) {
            logger.debug("User found: {}, Gender from DB (via CustomUserDetails.getUser().getGender()): {}",
                    customUserDetails.getUsername(), customUserDetails.getUser().getGender());
        } else {
            logger.warn("CustomUserDetails or its internal User object is null after loading for username: {}",
                    username);
        }
    ...
    public CustomUserDetails loadCustomUserByUsername(String username) {
        ...
        if (user.getGender() == null || user.getGender().trim().isEmpty()) {
            logger.warn("Gender is null or empty for user: {} AFTER loading from DB. This is likely the root cause.",
                    username);
        }
    ...
    public CustomUserDetails loadCustomUserByEmail(String email) {
        ...
        logger.info("User object loaded from DB for email: {}. Gender directly from user.getGender(): {}",
                user.getUsername(), user.getGender());

        if (user.getGender() == null || user.getGender().trim().isEmpty()) {
            logger.warn(
                    "Gender is null or empty for user (by email): {} AFTER loading from DB. This is likely the root cause.",
                    user.getUsername());
        }
