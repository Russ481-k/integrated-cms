...
public class ResourceNotFoundException extends CustomBaseException {
    ...
    // Constructor to use a more specific ErrorCode if available (e.g.,
    // USER_NOT_FOUND)
    public ResourceNotFoundException(ErrorCode specificErrorCode) {
        ...
        if (!specificErrorCode.getCode().startsWith("U") && !specificErrorCode.getCode().startsWith("E")
                && !specificErrorCode.getCode().startsWith("P")
                && !specificErrorCode.equals(ErrorCode.RESOURCE_NOT_FOUND)) {
            // This is a basic check; ideally, ResourceNotFoundException should only be used
            // with codes indicating a missing resource.
            // Consider logging a warning if a non-resource-not-found type ErrorCode is
            // used.
        }
    }
