package api.v2.cms.enterprise.service.impl;
...
import api.v2.cms.enterprise.dto.UpdateEnterpriseRequest;
import api.v2.cms.enterprise.repository.EnterpriseRepository;
import api.v2.cms.enterprise.service.EnterpriseService;
...
public class EnterpriseServiceImpl implements EnterpriseService {
    ...
    public Page<EnterpriseDto> getAllEnterprises(Integer year, String name, String representative, String businessType,
            Pageable pageable) {
    ...
    @Override
    public EnterpriseDto createEnterprise(CreateEnterpriseRequest createRequest, MultipartFile imageFile,
            String createdBy, String createdIp) {
        // 중복 체크 (동일 연도, 동일 이름)
        enterpriseRepository.findByNameAndYear(createRequest.getName(), createRequest.getYear())
                .ifPresent(e -> {
                    throw new DuplicateResourceException("Enterprise with name '" + createRequest.getName()
                            + "' already exists in year " + createRequest.getYear());
                });
            ...
            enterpriseRepository.findByNameAndYear(checkName, checkYear)
                    ...
                    .ifPresent(e -> {
                        throw new DuplicateResourceException(
                                "Enterprise with name '" + checkName + "' already exists in year " + checkYear);
                    });
        }
    ...
    private String processEnterpriseImage(Long enterpriseId, MultipartFile imageFile) {
        try {
            ...
            if (uploadedFiles != null && !uploadedFiles.isEmpty()) {
                ...
                log.debug("입주기업 이미지 업로드 완료: fileId={}, fileName={}", uploadedFile.getFileId(),
