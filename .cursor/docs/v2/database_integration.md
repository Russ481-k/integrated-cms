...
```mermaid
graph TD
    A["/api/v1/cms/integrated/users"] --> B[서비스 컨텍스트: integrated]
    B --> C[integrated_cms DB 직접 접근]
    C --> D[ADMIN_USER 테이블 조회]
    D --> E[결과 반환]
```
...
```mermaid
graph TD
    A["/api/v1/cms/service1/board/articles"] --> B[서비스 컨텍스트: service1]
    B --> C[integrated_cms.SERVICE 테이블 조회]
    C --> D[service1 DB 연결 정보 획득]
    D --> E[service1 DB 동적 연결]
    E --> F[board_articles 테이블 조회]
    F --> G[결과 반환]
```
...
### 4.1 동적 라우팅 시스템
- URL 패턴 기반 자동 서비스 식별
- 서비스 메타데이터를 통한 동적 DB 연결
- 통합 관리와 서비스별 관리의 명확한 분리
