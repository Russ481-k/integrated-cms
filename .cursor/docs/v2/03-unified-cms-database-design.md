...
CREATE TABLE MENU (
    ...
    SERVICE_ID VARCHAR(36) NULL COMMENT '서비스 ID (NULL이면 통합 관리용 메뉴)',
    ...
    MENU_TYPE VARCHAR(20) DEFAULT 'MENU' COMMENT '메뉴 타입 (MENU, PAGE, FUNCTION, BUTTON)',
    MENU_ICON VARCHAR(100) NULL COMMENT '메뉴 아이콘',
    ORGANIZATION_ID VARCHAR(36) COMMENT '소속 기관 ID',
    STATUS VARCHAR(20) DEFAULT 'ACTIVE' COMMENT '상태 (ACTIVE, INACTIVE, HIDDEN)',
    REQUIRED_PERMISSIONS JSON COMMENT '메뉴 접근에 필요한 권한 목록',
    ATTRIBUTES JSON COMMENT '메뉴 속성 정보',
    ...
    FOREIGN KEY (PARENT_MENU_ID) REFERENCES MENU(MENU_ID) ON DELETE CASCADE ON UPDATE CASCADE COMMENT '상위 메뉴 참조',
    FOREIGN KEY (CREATED_BY) REFERENCES ADMIN_USER(ADMIN_ID) ON DELETE SET NULL ON UPDATE CASCADE COMMENT '생성자 참조',
    FOREIGN KEY (UPDATED_BY) REFERENCES ADMIN_USER(ADMIN_ID) ON DELETE SET NULL ON UPDATE CASCADE COMMENT '수정자 참조',
